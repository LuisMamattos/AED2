...
int main() {
    ...
    int distancia[num_vertices], visitado[num_vertices], count, minimo, proximo;
    for(x=0;x<num_vertices;x++){
        distancia[x] = matriz_adj[origem-1][x];
        visitado[x] = 0;
    }
    ...
    while(count < num_vertices-1){
        minimo = 999;
        for(x=0;x<num_vertices;x++){
            if(distancia[x] < minimo && !visitado[x]){
                minimo = distancia[x];
                proximo = x;
            }
        }
        ...
        for(x=0;x<num_vertices;x++){
            if(!visitado[x]){
                if(minimo+matriz_adj[proximo][x] < distancia[x]){
                    distancia[x] = minimo+matriz_adj[proximo][x];
                }
            }
        }
        count++;
    }
    printf("\nA menor distancia entre os vertices %d e %d eh: %dKm", origem, destino, distancia[destino-1]);
